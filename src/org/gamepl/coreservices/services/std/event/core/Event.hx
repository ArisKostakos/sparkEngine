...
package org.gamepl.coreservices.services.std.event.core;
...
class Event extends AService implements IEvent
{
	...
	//an eventType IS an eventGroup PAIRED with a filter
	
	//nothing: every eventType will be linked to a specific filterType (string,Bool,object pointer, etc)
	
	
	//map: eventGroup flaga
	//private var _eventGroupFlags:Map<EEventGroup, Bool>;
	
	//map: each eventType will link to ONE event group (it's parent)
	//private var _eventTypeGroups:Map<EEventType, EEventGroup>;
	
	//map: each eventType will link to a flag (activate the eventType itself, ignoring filters)
	//private var _eventTypeFlags:Map<EEventType, Bool>;
	
	//map: each eventType will link to an eventFilter MAP	
	//eventFilter Map: a map of possible values for a filterType linked to flags. (<Dynamic,Bool>).
	//private var _eventTypeFilterFlags:Map<EEventType, Map<Dynamic, Bool>>;
	
	
	
	//private var _triggers:Map<EEventType,Array<IGameTrigger>>;
	
	//eventGroup Created
	//eventType Created
	//filter gameEntity
	
	//EventGroup KeyPressed
	//eventType KeyPressed_F
	//filter 'F'
	
	//Fake event Types for xml input
	//will create pairs of eventGroups and Filters
	//this is to place the triggers
	
	//in order to raise an event from anywhere, raise an eventGroup AND a filter
	//this will trigger everything in it
	
	
	//ok so we need some map here that will dientify all EventPrefabs, and resolve them to a pair of
	//an EVENT and a FILTER (according to the map and the eventPrefab directions..)
	//eg.: ep: KeyPressed_D:  Filter: "D"
	//eg2: ep: Created		  Filter: userEntity
	//dirtections check  <Dynamic>:   is it static inline string "xxxx"? no" is it static inline string "yyyy"? no? then store whatever it is in the filter map thing..
