	...
	var attachListners = function(container, curRul){
		...
		curRul.canvas.addEventListener('mousedown', function (e){
			...
			if(curRul.dimension === VERTICAL){
				guide.style.left = ruler.utils.pixelize(e.clientX - options.container.getBoundingClientRect().left); //Aris
			}
			else{
				guide.style.top = ruler.utils.pixelize(e.clientY - options.container.getBoundingClientRect().top); //Aris
			}
...
ruler.rulerConstructor =  function(_canvas, options, rulDimension)
{
	...
	var drawRuler = function (_rulerLength, _rulerThickness, _rulerScale){
		rulLength = canvas.width = _rulerLength * 40; //Aris, default was 4. This is dead code i think
